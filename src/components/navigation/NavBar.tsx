      ...
      <nav className="mx-auto max-w-7xl w-full px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <ul className="hidden md:flex gap-2 ml-4">
            ...
            <li title={t('navbar.books')} ><Link href="/libros" className={linkCls("Libros")} onClick={() => setSelected("Libros")}>{t('navbar.books')} </Link></li>
          ...
          <div title={t('navbar.language')} className="flex gap-2">
            {AVAILABLE_LOCALES.map((l) => (
              <button
                ...
                className={`cursor-pointer px-3 py-1 rounded ${locale === l ? "bg-[var(--colorMenus)] text-white" : "bg-gray-200"
                  }`}
              >
                {l.toUpperCase()}
              </button>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-elev bg-elev px-4 pb-4 space-y-3">
          ...
          <ul className="space-y-2">
            ...
            <li><Link href="/libros" className={linkCls("Libros")} onClick={() => setSelected("Libros")}>Libros</Link></li>
